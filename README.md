...
.cat-table th{text-align:left;padding:6px 8px;background:var(--primary-light);color:var(--primary);font-weight:700;font-size:11px;border-radius:4px}
...
<div id="app-shell" style="display:none">
  ...
  <div class="page active" id="page-dashboard">
    <div class="filter-bar" style="margin-top:8px">
      ...
      <button class="btn btn-outline btn-sm" onclick="loadAllData()" style="white-space:nowrap">🔄 Refresh</button>
    </div>
    <div class="kpi-grid">
    ...
    <div class="card">
      <div class="card-title">📊 รายรับ-รายจ่าย รายวัน</div>
      <div class="chart-wrap"><canvas id="dash-chart"></canvas></div>
    </div>
    <div class="card">
      <div class="card-title" style="color:var(--income)">💰 รายรับ แยกตามหมวดหมู่</div>
      <div class="cat-grid">
    ...
    <div class="card">
      <div class="card-title" style="color:var(--expense)">💸 รายจ่าย แยกตามหมวดหมู่</div>
      <div class="cat-grid">
...
const COLORS_INC = ['#10B981','#34D399','#059669','#6EE7B7','#047857','#A7F3D0','#065F46','#D1FAE5'];
...
function groupByCategory(recs){
  const map={};
  recs.forEach(r=>{
    if(!map[r.category])map[r.category]={cat:r.category,total:0,notes:[]};
    map[r.category].total+=r.amount;
    if(r.note&&!map[r.category].notes.includes(r.note))map[r.category].notes.push(r.note);
  });
...
function renderCatTable(containerId, catData, cssType){
  ...
  const rows=catData.map(d=>{
    ...
    return `<tr>
      <td><div style="font-weight:700">${d.cat}</div>${pills?`<div style="margin-top:3px">${pills}</div>`:''}</td>
...
function renderDashboard(){
  ...
  const expCat=groupByCategory(expRecs);
  ...
  if(expCat.length){
    expCatChart=renderCatChart('chart-exp-cat',expCat,COLORS_EXP);
    renderCatLegend('exp-cat-legend',expCat,COLORS_EXP);
