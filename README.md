...
.filter-bar select{padding:7px 10px;border:1.5px solid var(--border);border-radius:8px;font-size:12px;background:#fff;flex:1;min-width:100px}
...
<div id="app-shell" style="display:none">
  ...
  <div class="page active" id="page-dashboard">
    ...
    <div class="card">
      <div class="card-title" style="color:var(--expense)">💸 รายจ่าย แยกหมวดหมู่</div>
      <div class="cat-grid">
...
async function loadAllData(){
  ...
  try{
    ...
    populateAllSelects();renderEventList();renderDashboard();renderDetail();
...
function populateDashEventDayFilter(evId){
  const sel=document.getElementById('dash-event-day');
  const recs=evId?records.filter(r=>r.eventId===evId):records;
...
function populateDetDayFilter(evId){
...
function getFilteredRecords(){
  ...
  if(type==='day'){
    ...
    let f=evId?records.filter(r=>r.eventId===evId):[...records];
    if(day)f=f.filter(r=>r.date===day);
    return f;
  }
}
...
function renderCatChart(canvasId,catData,colors){
  ...
  return new Chart(ctx,{
    type:'doughnut',
    data:{labels:catData.map(d=>d.cat),datasets:[{data:catData.map(d=>d.total),backgroundColor:colors.slice(0,catData.length),borderWidth:2,borderColor:'#fff'}]},
...
function renderCatLegend(containerId,catData,colors){
  ...
  document.getElementById(containerId).innerHTML=catData.map((d,i)=>`
    <div class="cat-legend-item">
      ...
      <div><div class="cat-legend-label">${d.cat}</div>
      <div class="cat-legend-val">${fmt(d.total)} · ${total?Math.round(d.total/total*100):0}%</div></div>
    </div>`).join('');
}
function renderCatTable(containerId,catData,cssType){
  ...
  const rows=catData.map(d=>{
    ...
    return `<tr><td><div style="font-weight:700">${d.cat}</div>${pills?`<div style="margin-top:3px">${pills}</div>`:''}</td>
      <td><div>${fmt(d.total)}</div><div style="font-size:10px;color:var(--muted)">${pct}%</div></td></tr>`;
  }).join('');
...
function renderDashboard(){
  initDashDayFilter(); initDashMonthFilter();
  const type=document.getElementById('dash-type').value;
  document.getElementById('dash-day').style.display=type==='day'?'block':'none';
  document.getElementById('dash-month').style.display=type==='month'?'block':'none';
  document.getElementById('dash-event').style.display=type==='event'?'block':'none';
  ...
  // Bar chart
  const days={};
  filtered.forEach(r=>{if(!days[r.date])days[r.date]={inc:0,exp:0};
    if(r.type==='income')days[r.date].inc+=r.amount;else days[r.date].exp+=r.amount;
  });
  ...
  // Doughnut charts
  const incCat=groupByCategory(filtered.filter(r=>r.type==='income'));
  const expCat=groupByCategory(filtered.filter(r=>r.type==='expense'));
  if(incCatChart){incCatChart.destroy();incCatChart=null;}
  if(expCatChart){expCatChart.destroy();expCatChart=null;}
  if(incCat.length){incCatChart=renderCatChart('chart-inc-cat',incCat,COLORS_INC);renderCatLegend('inc-cat-legend',incCat,COLORS_INC);renderCatTable('inc-cat-table',incCat,'inc');}
  else{document.getElementById('inc-cat-legend').innerHTML='<div class="empty" style="padding:10px"><p>ไม่มีข้อมูล</p></div>';document.getElementById('inc-cat-table').innerHTML='';}
  if(expCat.length){expCatChart=renderCatChart('chart-exp-cat',expCat,COLORS_EXP);renderCatLegend('exp-cat-legend',expCat,COLORS_EXP);renderCatTable('exp-cat-table',expCat,'exp');}
  else{document.getElementById('exp-cat-legend').innerHTML='<div class="empty" style="padding:10px"><p>ไม่มีข้อมูล</p></div>';document.getElementById('exp-cat-table').innerHTML='';}
  // List
  ...
  list.innerHTML=[...filtered].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,30)
    .map(r=>`<div class="record-item"><div style="flex:1">
      <div class="record-label">${r.type==='income'?'💰':'💸'} ${r.category}</div>
      <div class="record-date">${r.date}</div>
      ${r.note?`<div class="record-note">📝 ${r.note}</div>`:''}
    </div><span class="record-amount ${r.type}">${r.type==='income'?'+':'-'}${fmt(r.amount)}</span></div>`).join('');
}
  ...
  let html=`<div class="card" style="background:linear-gradient(135deg,#7C3AED,#5B21B6);color:#fff">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
      <div style="background:rgba(255,255,255,.15);border-radius:10px;padding:10px;text-align:center">
        ...
        <div style="font-size:15px;font-weight:800">${fmt(totalInc)}</div>
      </div>
      <div style="background:rgba(255,255,255,.15);border-radius:10px;padding:10px;text-align:center">
        <div style="font-size:10px;opacity:.8">💸 รายจ่าย</div>
        <div style="font-size:15px;font-weight:800">${fmt(totalExp)}</div>
      </div>
      <div style="background:rgba(255,255,255,.25);border-radius:10px;padding:10px;text-align:center">
        <div style="font-size:10px;opacity:.8">📈 กำไร</div>
        <div style="font-size:15px;font-weight:800">${fmt(totalNet)}</div>
      </div>
    </div>
  </div>`;
  dates.forEach(date=>{
    ...
    const incCatDay=groupByCategory(dayRecs.filter(r=>r.type==='income'));
    const expCatDay=groupByCategory(dayRecs.filter(r=>r.type==='expense'));
    html+=`<div class="detail-day-block">
      <div class="detail-day-head">
        <div><div class="date-lbl">📅 ${date}</div>${eventName?`<div style="font-size:10px;opacity:.8">🎪 ${eventName}</div>`:''}</div>
    ...
    if(incCatDay.length){
      html+=`<div class="detail-section">
        ...
        ${incCatDay.map(d=>`<div class="detail-row">
          <div style="flex:1"><div class="detail-row-cat">${d.cat}</div>${d.notes.length?`<div class="detail-row-note">📝 ${d.notes.join(', ')}</div>`:''}</div>
    ...
    if(expCatDay.length){
      html+=`<div class="detail-section">
        <div class="detail-section-title" style="color:var(--expense)">💸 รายจ่าย — ${fmt(dExp)}</div>
